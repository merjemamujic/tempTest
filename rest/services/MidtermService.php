    ...
    */
    public function cap_table(){
        ...
        foreach($initialData as $row){
            ...
            $share_class_category = $this->dao->getShareClassCategory($share_class_category__id);
            $investor = $this->dao->getInvestors($investor_id);

            $result[$share_class_id]['class'] =  $share_class['description'];
            $result[$share_class_id]['categories'][$share_class_category__id]['category'] = $share_class_category['description'];
            $result[$share_class_id]['categories'][$share_class_category__id]['investors'][$investor_id]['investor']= 
                $investor['first_name'] . ' ' .$investor['last_name'];
            $result[$share_class_id]['categories'][$share_class_category__id]['investors'][$investor_id]['diluted_shares'] = 
                $row['diluted_shares'];
        }
